...
def horizontal_cut_chars(plate):
    char_addr_list = [] #初始化一个空列表t，用于存储每个字符的左右边界索引
    ...
    for i in range(img_w):
        ...
        if colValue > col_limit: # 每列边缘像素点必须超过均值的60%才能判断属于字符区域
...
def get_chars(car_plate):
    ...
    h_proj_limit = [0.2,0.8] # 设置水平投影的长度阈值，用于过滤掉不符合车牌特征的水平投影区域
    char_imgs = []

    # 遍历车牌图像的每一行，统计每行中值为 255（白色）的像素数量，并存储在 h_count 中
    ...
    for row in range(img_h):
        ...
        if temp_cnt/img_w<h_proj_limit[0] or temp_cnt/img_w>h_proj_limit[1]: #如果当前行的白色像素比例小于 20% 或大于 80%，则认为该行不是车牌的有效区域
            if h_temp_len != 0:
                h_end_index = row-1
    ...
    # 如果遍历完图像后，还有未结束的水平投影，则将其添加到 h_proj_list 中
    if h_temp_len != 0:
        h_end_index = img_h-1
        h_proj_list.append((h_startIndex, h_end_index))
